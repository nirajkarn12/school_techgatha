...
if(isset($_POST['form3'])) {
    ...
    if ($statement->rowCount() > 0) {
        $statement = $pdo->prepare("UPDATE tbl_settings SET newsletter_on_off=?, footer_copyright=?, contact_address=?, contact_email=?, contact_phone=?, contact_map_iframe=?, marquee_on_off=?, marquee_notices=? WHERE id=1");
        $statement->execute(array($_POST['newsletter_on_off'],$_POST['footer_copyright'],$_POST['contact_address'],$_POST['contact_email'],$_POST['contact_phone'],$_POST['contact_map_iframe'], $marquee_on_off, $marquee_notices));
    } else {
...
if(isset($_POST['form12'])) {
    $commissionType = $_POST['default_staff_commission_type'] ?? 'percent';
    $commissionValue = (float)($_POST['default_staff_commission_value'] ?? 35);
    if (!in_array($commissionType, array('percent', 'fixed'), true)) {
        $commissionType = 'percent';
    }
    try {
        $statement = $pdo->prepare("UPDATE tbl_settings SET default_staff_commission_type=?, default_staff_commission_value=? WHERE id=1");
        $statement->execute(array($commissionType, $commissionValue));
        $success_message = 'Staff commission default is updated successfully.';
    } catch (PDOException $e) {
        $error_message .= 'Could not update staff commission settings. Run Phase 2 migration first.<br>';
    }
}
...
foreach ($result as $row) {
    $logo                            = $row['logo'];
    $favicon                         = $row['favicon'];
    $site_name                       = $row['site_name'] ?? 'Techgatha School';
 ...
 //   $stripe_secret_key               = $row['stripe_secret_key'];
    ...
    $default_staff_commission_value  = $row['default_staff_commission_value'] ?? 35;
                            ...
                            <div class="box box-info">
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-3">
                                            <select name="marquee_on_off" class="form-control" style="width:auto;">
                                                <option value="1" <?php if($marquee_on_off == 1) {echo 'selected';} ?>>On</option>
                                                <option value="0" <?php if($marquee_on_off == 0) {echo 'selected';} ?>>Off</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="" class="col-sm-2 control-label">Marquee Notices </label>
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="marquee_notices" rows="5" placeholder="One notice per line"><?php echo htmlspecialchars($marquee_notices); ?></textarea>
                                            <p class="help-block">Shown in the scrolling bar below the homepage hero. Enter one notice per line.</p>
                                        </div>
                                    </div>
                                    <div class="form-group">
                        ...
                        <div class="tab-pane" id="tab_12">
                            <form class="form-horizontal" action="" method="post">
                                <div class="box box-info">
                                    <div class="box-body">
                                        <div class="form-group">
                                            <label for="" class="col-sm-2 control-label">Default Commission Type</label>
                                            <div class="col-sm-4">
                                                <select name="default_staff_commission_type" class="form-control">
                                                    <option value="percent" <?php echo (($default_staff_commission_type ?? 'percent') === 'percent') ? 'selected' : ''; ?>>Percentage</option>
                                                    <option value="fixed" <?php echo (($default_staff_commission_type ?? '') === 'fixed') ? 'selected' : ''; ?>>Fixed Amount</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="col-sm-2 control-label">Default Commission Value</label>
                                            <div class="col-sm-4">
                                                <input type="number" step="0.01" min="0" name="default_staff_commission_value" class="form-control" value="<?php echo htmlspecialchars($default_staff_commission_value ?? '35'); ?>">
                                                <p class="help-block">Used when service and staff do not set a commission rule. Example: 35 = 35%.</p>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="" class="col-sm-2 control-label"></label>
                                            <div class="col-sm-6">
                                                <button type="submit" class="btn btn-success pull-left" name="form12">Update</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
