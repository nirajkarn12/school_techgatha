...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement->execute(array($_POST['caption'],$id));
			$success_message = 'Photo is updated successfully.';
    	} else {
			$ext = strtolower($ext);
			$old_photo = $_POST['previous_photo'] ?? '';
			$final_name = adminUniqueUploadName('photo', $ext, $id);
			if (!adminMoveUploadedFile($path_tmp, $final_name)) {
				...
				$statement->execute(array($_POST['caption'],$final_name,$id));
				if ($old_photo !== '' && $old_photo !== $final_name) {
					adminDeleteUploadIfUnused($pdo, $old_photo, 'tbl_photo', 'photo', $id);
				}
				$success_message = 'Photo is updated successfully.';
			}
    	}
