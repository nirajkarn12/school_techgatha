...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
    		...
    		$statement->execute(array($_POST['heading'],$_POST['content'],$_POST['button_text'],$_POST['button_url'],$_POST['position'],$id));
			$success_message = 'Slider is updated successfully!';
		} else {
			$ext = strtolower($ext);
			$old_photo = $_POST['current_photo'] ?? '';
			$final_name = adminUniqueUploadName('slider', $ext, $id);
			if (!adminMoveUploadedFile($path_tmp, $final_name)) {
				...
				$statement->execute(array($final_name,$_POST['heading'],$_POST['content'],$_POST['button_text'],$_POST['button_url'],$_POST['position'],$id));
				if ($old_photo !== '' && $old_photo !== $final_name) {
					adminDeleteUploadIfUnused($pdo, $old_photo, 'tbl_slider', 'photo', $id);
				}
				$success_message = 'Slider is updated successfully!';
			}
		}
